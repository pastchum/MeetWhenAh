...
CREATE TABLE users (
    ...
    tele_id BIGINT unique, -- Telegram numeric ID (if any)
...
CREATE TABLE events (
    ...
    creator UUID NOT NULL REFERENCES users (uuid) ON DELETE RESTRICT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
create policy "service only" on webapp_share_tokens for all using (true)
with
    check (true);
